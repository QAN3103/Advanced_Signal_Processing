...
def spectral_difference_bootstrap(original_signal, processed_signal, plot):
    l = np.arange(0,10)
    k_coeff = np.arange(1,11)
    n_spec = np.linspace(0,1,80)
    ...
    print(hist_matrix)
    np.savetxt("txtfiles/rms_SNR3.dat",hist_matrix,fmt="%f", delimiter="\t",header="freq\trms", comments="")
    rms_median = np.median(rms_star)
    med_rms = np.column_stack((0.1,rms_median))
    np.savetxt("txtfiles/rms_median_SNR3.dat",med_rms,fmt="%f", delimiter="\t",header="freq\trms", comments="")
    
    # Spectral Difference
    txt = np.column_stack((n_spec,median_V, lower_bound_V, upper_bound_V))
    np.savetxt("txtfiles/spectral_difference_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tmedian\tlowerbound\tupperbound", comments="")
    ...
    txt = np.column_stack((k_coeff,med,y_err_low,y_err_high))
    np.savetxt("txtfiles/ar_conf_orig_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tmedian\terrorlow\terrorhigh", comments="")
    ...
    for i in range(len(low)):
        ...
        y_err_high[i] = high[i] - med[i]
    txt = np.column_stack((k_coeff,med,y_err_low,y_err_high))
    np.savetxt("txtfiles/ar_conf_proc_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tmedian\terrorlow\terrorhigh", comments="")
    
    # Parcor Coeffs
    low = lower_bound_parcor_orig
    high = upper_bound_parcor_orig
    med = median_parcor_orig
    y_err_low = np.empty(len(low))
    y_err_high = np.empty(len(high))
    for i in range(len(low)):
        y_err_low[i] = med[i] - low[i]
        y_err_high[i] = high[i] - med[i]
    txt = np.column_stack((k_coeff,med,y_err_low,y_err_high))
    np.savetxt("txtfiles/parcor_conf_orig_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tmedian\terrorlow\terrorhigh", comments="")
    low = lower_bound_parcor_proc
    high = upper_bound_parcor_proc
    med = median_parcor_proc
    y_err_low = np.empty(len(low))
    y_err_high = np.empty(len(high))
    for i in range(len(low)):
        y_err_low[i] = med[i] - low[i]
        y_err_high[i] = high[i] - med[i]
    txt = np.column_stack((k_coeff,med,y_err_low,y_err_high))
    np.savetxt("txtfiles/parcor_conf_proc_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tmedian\terrorlow\terrorhigh", comments="")
    ###
...
if __name__ == "__main__":
    plot = False
    n = np.arange(1,161)
    ch_sound,_ = pp1.load_wave('audio/ch_sound.wav',True, plot=plot, normalize=True)
    noise,_ = pp1.load_wave('audio/vehicle-movement-noise.wav',True, plot=plot, normalize=True)
    
    # Calculate noisy signal and processed signal
    snr = 3
    noisy_signal = contaminate_signal_with_noise(ch_sound, noise, snr)
    processed_signal = wiener(noisy_signal)
    # Save signals to datfiles
    txt = np.column_stack((n,noisy_signal))
    np.savetxt("txtfiles/noisy_signal_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tvalue", comments="")

    txt = np.column_stack((n,processed_signal))
    np.savetxt("txtfiles/processed_signal_SNR3.dat",txt,fmt="%f", delimiter="\t",header="index\tvalue", comments="")
    
    # Bootstrap
    spectral_difference_bootstrap(ch_sound,processed_signal,plot)
